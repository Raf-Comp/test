...
/**
 * Klasa odpowiedzialna za aktywację wtyczki
 */
class Activator {
    /**
     * @var SecurityService
    ...
    public function activate(): void {
        try {
            ...
            wp_redirect(admin_url('admin.php?page=aica-settings&activated=true'));
            exit;
        } catch (\Throwable $e) {
            // Loguj błąd
            error_log(sprintf(
                'AICA Plugin Activation Error: %s in %s:%d',
                $e->getMessage(),
                $e->getFile(),
                $e->getLine()
            ));
            ...
            wp_die(
                sprintf(
                    __('Błąd aktywacji wtyczki AI Chat Assistant: %s', 'ai-chat-assistant'),
                    $e->getMessage()
                ),
    ...
    private function check_requirements(): void {
        ...
        if (!current_user_can('activate_plugins')) {
            throw new \RuntimeException(
                __('Nie masz uprawnień do aktywacji wtyczki.', 'ai-chat-assistant')
            );
        }
    }
    ...
    private function set_default_options(): void {
        ...
        add_option('aica_settings', $default_options);
    }
